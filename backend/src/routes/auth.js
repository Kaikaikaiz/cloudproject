...
const safe = (user) =>
  Object.fromEntries(Object.keys(publicUser).map((key) => [key, user[key]]));
router.post('/register', limit, async (req, res) => {
  ...
  if ('role' in req.body || 'id' in req.body)
    fail('Account roles cannot be selected during registration.');
  if (values.email === process.env.ADMIN_EMAIL?.trim().toLowerCase())
    fail('This email is reserved. Please use a different email.', 409);
...
router.post('/forgot-password', limit, (req, res) => {
  ...
  res.json({
    message:
      'Local preview: your request has been received. No email is sent and your password has not changed. Contact the local project administrator for help.',
  });
});
router.get('/admin', requireAuth, requireAdmin, (_req, res) =>
  res.json({ message: 'Administrator access verified.' }),
);
