...
export default function PaymentSuccess({ kind }) {
    ...
    api(endpoint)
      .then((data) => {
        ...
        if (receipt.status !== 'SUCCESS') {
          throw new Error('This payment has not completed. Check its status in your wallet.');
        }
      ...
      <section className="page-section payment-page">
        <div className="surface payment-panel">
          ...
          <div className="profile-actions">
            ...
            <Button to="/wallet" variant="secondary">Back to wallet</Button>
          </div>
        </div>
      </section>
    );
  ...
  const title = isTopUp ? 'Top-up successful!' : isSeller ? 'Payment received!' : 'It’s yours!';
  ...
  return (
    <section className="page-section payment-page">
      <div className="surface payment-panel payment-success">
        ...
        <h1>{title}</h1>
        <p>{isTopUp ? 'A little more room for your next favourite.' : payment.listingTitle}</p>
        <strong className="payment-total">{rm(payment.amount)}</strong>
        <p className="payment-mock-notice">Simulated payment complete · No real money was moved.</p>

        <dl className="payment-details">
          <div><dt>Method</dt><dd>{paymentMethodLabels[payment.method]}</dd></div>
          {!isTopUp && <div><dt>{isSeller ? 'Buyer' : 'Seller'}</dt><dd>{isSeller ? payment.buyer.name : payment.seller.name}</dd></div>}
          <div><dt>Date</dt><dd>{new Date(payment.createdAt).toLocaleString('en-MY')}</dd></div>
          <div><dt>Reference</dt><dd className="payment-reference">{payment.reference}</dd></div>
        </dl>

        <div className="profile-actions">
          {isTopUp && returnTo && <Button to={returnTo}>Return to checkout</Button>}
          <Button to="/wallet" variant={returnTo ? 'secondary' : 'primary'}>View wallet</Button>
          <Button to="/" variant="ghost">Explore more finds</Button>
        </div>
      </div>
    </section>
  );
